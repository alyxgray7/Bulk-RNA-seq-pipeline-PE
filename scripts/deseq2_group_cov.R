...
p <- pheatmap(sampleDistMatrix, fontsize=5, scale="row",
         clustering_distance_rows=sampleDists,
         clustering_distance_cols=sampleDists,
         col=colors)
save_pheatmap_pdf(p, distance_plot)
# dev.off()
...
# pdf(heatmap_plot)
p <- pheatmap(assay(rld)[topGenes,], cluster_rows=T, scale="row", fontsize=6,fontsize_row=6, fontsize_col=6, show_rownames=T, cluster_cols=T, annotation_col=annot, labels_col=as.character(rownames(df)), main = paste("Heatmap of top 50 DE genes across all samples"))
save_pheatmap_pdf(p, heatmap_plot)
# dev.off()
...
#   dds.lrt <- DESeq(dds, test = "LRT", reduced=~1)
  ...
  p <- pheatmap(assay(rld)[topGenes,], cluster_rows=T, scale="row", fontsize=6,fontsize_row=6,fontsize_col=6, show_rownames=T, cluster_cols=T, annotation_col=annot, labels_col=as.character(rownames(df)), main = paste("Heatmap of top 50 DE genes across selected samples"))
  # dev.off()
  save_pheatmap_pdf(p, sub("$", "/subsetted_heatmap.pdf", paste0(io$outDir)))
}
